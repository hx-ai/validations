...
class ModelParser {
    ...
    if (fields != null) {
      for (var name in fields.keys) {
        ...
        final annotatedField = ValidatedElement(
          ...
          type: propertyField.type.element.displayName,
          library: library,
    ...
    for (var field in fields) {
      ...
      if (hasValidatorAnnotations) {
        ...
        final annotatedField = ValidatedElement(
          ...
          elementType: ElementType.FIELD,
          modelClass: model.element as ClassElement,
          type: field.type.element.displayName,
          library: library,
    ...
    if (field.usedAtClassLevel) {
      ...
      parameters.addAll([
        ...
        Parameter(
          (parameter) => parameter
            ..name = 'object'
            ..type = refer(model.element.displayName),
        )
      ]);
    } else {
  ...
  Method _buildPropsMethod(List<ValidatedElement> annotatedFields) {
    ...
    final body = refer('PropertyMap<${model.element.displayName}>')
        .newInstance(
          [
    ...
    return Method(
      (MethodBuilder builder) {
        builder
          ...
          ..returns = refer('PropertyMap<${model.element.displayName}>')
  ...
  List<Expression> _buildPositionalArguments(
    ...
    if (annotation.isContainerAnnotation) {
      ...
      final str = refer('${containerValidator.thisType.element.displayName}()..validationContext = validationContext,');
  ...
  ClassElement _getValidatorForModel(ClassElement validatedModel) {
    ...
    for (var annotatedElement in library.annotatedWith(genValidatorType)) {
      ...
      if (model.element.displayName == validatedModel.name) {
